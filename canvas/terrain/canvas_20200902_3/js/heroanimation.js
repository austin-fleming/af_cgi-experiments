...
const FIELD_SCALE_FACTOR = 30;
const FIELD_X_OFFSET = 0;
const FIELD_Y_OFFSET = -700;
const FIELD_MAGNITUDE = 30;
const WAVE_SPEED = .2;
//const COLOR_01 = '#F1DEDE';
const COLOR_01 = 'rgba(255,230,230,0.7)';
...
const noiseFilter = (fieldArray, maxMagnitude, perlin, zOffset) => {
    ...
    for (let y = 0; y < fieldArray.length; y++) {
        for (let x = 0; x < fieldArray[y].length; x++) {
            ...
            fieldArray[y][x].y += perlin.get(fieldArray[y][x].x, fieldArray[y][x].y + noiseOffset) *maxMagnitude; */
            fieldArray[y][x].y = fieldArray[y][x].yReset;
            fieldArray[y][x].y += perlin.noise(fieldArray[y][x].x, fieldArray[y][x].y, fieldArray[y][x].z + zOffset) * maxMagnitude;
        }
    }
...
const POSITIONED_FIELD = convertCartesianFieldToIsometric(positionField(scaleField(CART_POINT_FIELD, FIELD_SCALE_FACTOR), FIELD_X_OFFSET, FIELD_Y_OFFSET));

var perlin = new toxi.math.noise.PerlinNoise();

//perlin.noiseDetail(0.1);
...
const animationLoop = _ => {
    ...
    noiseFilter(POSITIONED_FIELD, FIELD_MAGNITUDE, perlin, ticker).map(row => ffspline.renderSpline(CTX, row, SPLINE_TENSION, COLOR_01, 10));
